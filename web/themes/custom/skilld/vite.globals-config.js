...
export const cssBuild = (inputPrefixPath = '') => {
  return {
    ...
    build: {
      ...
      minify: false,
      rollupOptions: {
        input: sync(`${inputPrefixPath}/**/*.src.css`, { ignore: `${inputPrefixPath}/**/_*.src.css` }),
        output: {
          dir: '.',
          assetFileNames: ({ name }) => {
            const ext = extname(name);
            if (ext === '.css') {
              return sync(`${inputPrefixPath}/**/*.src.css`, { ignore: `${inputPrefixPath}/**/_*.src.css` })
  ...
  return {
    ...
    build: {
      ...
      rollupOptions: {
        ...
        `,
          entryFileNames: ({ name }) =>
            sync(`${inputPrefixPath}/**/*.src.js`).find((a) => a.includes(basename(name))).replace('.src', ''),
        },
      },
    },
  }
