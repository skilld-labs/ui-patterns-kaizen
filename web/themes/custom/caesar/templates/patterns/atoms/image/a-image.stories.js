...
import { faker } from '@caesar_sb/plugins/caesar';
...
export const getImageProperties = (obj) => {
  ...
  if (obj.effect === 'image_scale') {
    if (obj.width === 'auto') {
      props.width = randomVal(
        Math.floor(obj.height * 0.65),
        Math.floor(obj.height * 1.35),
      );
      props.height = obj.height;
    } else if (obj.height === 'auto') {
      props.width = obj.width;
      props.height = randomVal(
        Math.floor(obj.width * 0.65),
        Math.floor(obj.width * 1.35),
      );
    }
  }
...
export default {
  ...
  render: (args) => {
    ...
    componentSource.styles.every((style) => {
      if (
        (args.style && style.label === args.style) ||
        (args.name && style.name === args.name)
      ) {
        matched = style;
        return false;
      }
