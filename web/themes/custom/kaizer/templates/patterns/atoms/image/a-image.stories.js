...
export const Image = {
  render: (args = {}) => {
    const { data, template } = defRender(args, {
      a_image: {
  ...
  render: (args = {}) => {
    ...
    const responsiveImageGroup =
      Object.keys(responsiveImageGroups).find(
        (key) => responsiveImageGroups[key].label === args.group,
    ...
    data.img_element = `<img storybook-fallback-image-style-name="${
      responsiveImageGroups[responsiveImageGroup].fallback
    }" ${args.alt ? `alt="${args.alt}"` : ''} src="${faker.image[fakerProvider](
      fallback.width,
      fallback.height,
    )}"/>`;
    data.sources = [];
    ...
    Object.keys(breakpoints).forEach((breakpoint) => {
      ...
      data.sources.push(
        `storybook-breakpoint-name="${breakpoint}" ${help} srcset="${srcset}" media="${window.drupalSettings.kaizerMediaBreakpoints[breakpoint]}"`,
      );
    });
