...
const defContent = () => {
  ...
  for (let i = 1; i <= 10; i++) {
    items.push(
      `<div style="text-align: center; padding: 16px; background-color: var(--color-grey)">Slide ${i}</div>`,
    );
  }
  return items;
};
...
const BasicRender = (args) => {
  ...
  if (!data.type) {
    data.type = Object.keys(
      Object.values(description)[0].settings.type.options,
    )[0];
  }
