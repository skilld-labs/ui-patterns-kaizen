...
import <%= h.changeCase.lower(component_type).charAt(0) %><%= h.inflection.camelize(name) %>Yml from <% if (is_pattern === 'yes') { %>'./<%= h.changeCase.lower(component_type).charAt(0) %>_<%= h.changeCase.lower(h.inflection.dasherize(name)) %>.ui_patterns.yml'<% } else { %>'./<%= h.changeCase.lower(component_type).charAt(0) %>_<%= h.changeCase.lower(h.inflection.dasherize(name)) %>.yml'<% } %>;
...
export default {
  ...yml.<%= h.changeCase.lower(component_type).charAt(0) %>_<%= h.changeCase.lower(h.inflection.dasherize(name)) %>.story_settings.default,
  parameters: {
...
const basicRender = (args) => {
  const data = {
    ...
    ...window.decodeArgType(args, yml.<%= h.changeCase.lower(component_type).charAt(0) %>_<%= h.changeCase.lower(h.inflection.dasherize(name)) %>.settings),
  };
  data.content = args.content || yml.<%= h.changeCase.lower(component_type).charAt(0) %>_<%= h.changeCase.lower(h.inflection.dasherize(name)) %>.fields.content.preview;
  useEffect(() => {
