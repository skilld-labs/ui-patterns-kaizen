...
- This package can not be used as standalone. It's only working as a part of [ui-patterns-kaizen-tg](https://www.npmjs.com/package/@skilld/ui-patterns-kaizen-tg) package.
...
```
useEffect(() => {
  Drupal.behaviors.ui_patterns_kaizen_core_h_entity_fake_link.attach();
}, [args]);
```
...
If you need to specify your custom configuration for drupal's usage, please see how it can be done from `themename/src/js/h-entity-fake-link.js`.
...
This helper component is included globally already in storybook, when you installing [@skilld/ui-patterns-kaizen-tg](https://www.npmjs.com/package/@skilld/ui-patterns-kaizen-tg) theme. And also this script is included globally in `src/` folder (so Drupal have this script loaded by default too). You don't need to do anything manually with it.
