---
to: <%= h.src() %>/<%= h.changeCase.lower(name) %>/color/preview.js
---
/**
 * @file
    ...
    callback: function callback(context, settings, form) {
      ...
      if (colorPreview.length && colorPalette) {
        for (let i = 0; i < colorPreview.length; i++) {
          colorPreview[i].style.backgroundColor = colorPalette.querySelector('input[name="palette[' +
            colorPreview[i].getAttribute('class') + ']"]').value
        }
      }
    }
  };
